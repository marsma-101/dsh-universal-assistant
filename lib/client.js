/**
 * dsh-universal-assistant — client half (built bundle), v0.2.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const API = '/universal-assistant-api'
		const ENTRY_ID = 'universal-assistant'
...
`
		...
		function ensureStyles() {
			if (document.getElementById('dsh-universal-assistant-css') !== null) return
			const tag = document.createElement('style')
			tag.id = 'dsh-universal-assistant-css'
			tag.textContent = CSS
		...
		exports.name = 'universal-assistant'
		...
		exports.apply = function apply(ctx) {
			...
			slots.inject('shell.overlay', () =>
				slots.register({ name: 'shell.overlay', id: 'universal-assistant-dialog', order: 50 }, () =>
